...
public class PermissionManager : Repository<Permission>
{
    ...
    //这里面写的代码不会给覆盖,如果要重新生成请删除 PermissionManager.cs
    ...
    public void Study()
    {
	   ...
	   /*********查询*********/
        ...
        conModels.Add(new ConditionalModel() { FieldName= typeof(Permission).GetProperties()[0].Name, ConditionalType = ConditionalType.Equal, FieldValue = "1" });//id=1
        ...
        /*********插入*********/
        var insertData = new Permission() { };//测试参数
        var insertArray = new Permission[] { insertData };
		...
		/*********更新*********/
	    var updateData = new Permission() {  };//测试参数
        var updateArray = new Permission[] { updateData };//测试参数
        base.Update(updateData);//根据实体更新
        base.UpdateRange(updateArray);//批量更新
        //base.Update(it => new Permission() { ClassName = "a", CreateTime = DateTime.Now }, it => it.id==1);// 只更新ClassName列和CreateTime列，其它列不更新，条件id=1
        base.AsUpdateable(updateData).ExecuteCommand();  //转成Updateable可以实现复杂的插入



		/*********删除*********/
	    var deldata = new Permission() {  };//测试参数
