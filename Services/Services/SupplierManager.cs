...
{
public class SupplierManager : Repository<Supplier>
{
        ...
        public void Study()
    {
	   ...
	   /*********查询*********/
        ...
        conModels.Add(new ConditionalModel() { FieldName= typeof(Supplier).GetProperties()[0].Name, ConditionalType = ConditionalType.Equal, FieldValue = "1" });//id=1
        ...
        /*********插入*********/
        var insertData = new Supplier() { };//测试参数
        var insertArray = new Supplier[] { insertData };
		...
		/*********更新*********/
	    var updateData = new Supplier() {  };//测试参数
        var updateArray = new Supplier[] { updateData };//测试参数
        base.Update(updateData);//根据实体更新
        base.UpdateRange(updateArray);//批量更新
        //base.Update(it => new Supplier() { ClassName = "a", CreateTime = DateTime.Now }, it => it.id==1);// 只更新ClassName列和CreateTime列，其它列不更新，条件id=1
        base.AsUpdateable(updateData).ExecuteCommand();  //转成Updateable可以实现复杂的插入



		/*********删除*********/
	    var deldata = new Supplier() {  };//测试参数
