...
{
public class UserRoleManager : Repository<UserRole>
{
        public List<int> GetUserIDsByRole(Role role)
        {
            List<int> userIDs = Db.Queryable<UserRole>()
                    .InnerJoin<User>((ur, u) => ur.UserID == u.Id)
                    .Where((ur, u) => ur.RoleID == role.Id)
                    .OrderBy((ur, u) => u.UserName)       // 在 Select 之前指定多表 OrderBy
                    .Select((ur, u) => u.Id)
                    .ToList();
            return userIDs;

        }
        public bool SaveData(Role role, List<User> users)
        {
            if (role == null)
            {
                return false;
            ...
            Db.Ado.UseTran(() =>
            {
                var listToInsert = new List<UserRole>();

                // 删除该角色已有的用户映射（在事务内执行）
                base.Delete(it => it.RoleID == role.Id);
                foreach (var user in users)
                {
                    listToInsert.Add(new UserRole
                    {
                        UserID = user.Id,
                        RoleID = role.Id
                    });
        ...
        public void Study()
    {
	   ...
	   /*********查询*********/
        ...
        //base.Update(it => new UserRole() { ClassName = "a", CreateTime = DateTime.Now }, it => it.id==1);// 只更新ClassName列和CreateTime列，其它列不更新，条件id=1
