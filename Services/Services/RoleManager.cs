...
public class RoleManager : Repository<Role>
{
        ...
        {
            return base.GetList(it => it.RoleName == roleName).Count > 0;
        }
        public bool DeleteRole(Role role)
        {
            ...
            try
            {
                ...
                {
                    // 先删除该角色的映射
                    Db.Deleteable<RolePermission>().Where(rp => rp.RoleID == role.Id).ExecuteCommand();

                    Db.Deleteable<UserRole>().Where(ur => ur.RoleID == role.Id).ExecuteCommand();
                    // 再删除该角色
                    Db.Deleteable<Role>().Where(r => r.Id == role.Id).ExecuteCommand();
                });
                return true;
            }
            catch
        ...
        //这里面写的代码不会给覆盖,如果要重新生成请删除 RoleManager.cs
        ...
        public void Study()
    {
	   ...
	   /*********查询*********/
        ...
        conModels.Add(new ConditionalModel() { FieldName= typeof(Role).GetProperties()[0].Name, ConditionalType = ConditionalType.Equal, FieldValue = "1" });//id=1
        ...
        /*********插入*********/
        var insertData = new Role() { };//测试参数
        var insertArray = new Role[] { insertData };
		...
		/*********更新*********/
	    var updateData = new Role() {  };//测试参数
        var updateArray = new Role[] { updateData };//测试参数
        base.Update(updateData);//根据实体更新
        base.UpdateRange(updateArray);//批量更新
        //base.Update(it => new Role() { ClassName = "a", CreateTime = DateTime.Now }, it => it.id==1);// 只更新ClassName列和CreateTime列，其它列不更新，条件id=1
        base.AsUpdateable(updateData).ExecuteCommand();  //转成Updateable可以实现复杂的插入



		/*********删除*********/
	    var deldata = new Role() {  };//测试参数
